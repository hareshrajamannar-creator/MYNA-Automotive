...
| CoachCopilot | components/CoachCopilot/CoachCopilot.tsx | messages (CopilotMessage[]), changes (ProposedChange[]), changeDecisions, onDecideChange(changeId,decision), onOpenChange?(changeId), onSend(text), busy?, composerPlaceholder? — reusable Podium/Sierra-style copilot chat surface: user bubbles, assistant prose, collapsible "Thoughts", tool-step cards, compact change-proposed card (click → `ChangeDetailDrawer` side pane), test-run card (original vs revised + Keep original / Accept), follow-up composer. Data types in `src/data/feedbackData.ts`; used by `CoachCopilotScreen` (agent Feedback tab + inbox coaching); designed to be reused by a future create-agent-with-copilot flow |
| ChangeDetailDrawer | components/ChangeDetailDrawer/ChangeDetailDrawer.tsx | open, change (ProposedChange \| null), onClose — 650px right side pane showing a proposed change in full: "Why this fixes it" banner + per-target view. `workflowView` → simplified node chain with the edited step highlighted ("Edited" chip, branch pills) + field diffs; `procedureView` → procedure-library-style read-only view (when to use, numbered steps with added/removed bullet highlights, tool RefChips); fallback → plain red/green diff list |
...
- **Feedback + coaching (revamp of the old Recommendation tab):** `AgentInstanceScreen` tab is now **Feedback** → `FeedbackTab.tsx` (DataTable of `FeedbackRecord`s — AI recommendations + coaching sessions, status/search filters) → row click opens `CoachCopilotScreen.tsx` full-screen takeover (centered `CoachCopilot` chat + right "Changes" rail; accept/reject flips record status via `FeedbackStoreContext`). Inbox has conversation-level coaching: header **Coach** button + per-agent-message "Coach" link / thumbs-down → `ShareFeedbackModal` → submitting files a coaching record (`buildCoachingSession` in `feedbackData.ts`) **without leaving the inbox** — a toast with "Track feedback" and a per-message "Track submitted feedback" link open the copilot takeover (staged playback). Change cards open `ChangeDetailDrawer` (workflow node-chain view or procedure-style view). Reminder agent records demonstrate workflow-config edits (reminder message text on the "Appointment reminder" step; starting procedure on "Initiate voice call", the embedded voice agent) paired with a procedure update. Old `RecommendationsTab.tsx` / `FrontdeskRecommendationsTab.tsx` / `RecommendationsScreen.tsx` are no longer rendered (kept for reference).
