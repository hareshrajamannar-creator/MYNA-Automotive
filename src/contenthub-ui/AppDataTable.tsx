...
import { AppDataTableColumnSettingsTrigger } from "./AppDataTableColumnSettingsTrigger";
import {
  flexRender,
  getCoreRowModel,
  getSortedRowModel,
...
} from "./appDataTableRowDensity";
import {
  buildLayoutInputsFromColumnDefs,
  buildSizeWeightsFromColumnDefs,
  distributeWidths,
  resolveWeights,
} from "./appDataTableColumnSizing";
import { ColumnSettingsSheet, type ColumnSettingsSheetColumn } from "./ColumnSettingsSheet";
...
const STICKY_LEADER_CELL_BASE =
  "sticky border-r border-transparent bg-background transition-[box-shadow,border-color] duration-200 ease-out overflow-visible group-hover/table-row:bg-surface-hover group-data-[state=selected]/table-row:bg-surface-selected";
                      ...
                      {header.isPlaceholder ? null : (
                        <div className="flex min-w-0 items-center gap-1 pr-2">
                          {canSort ? (
                            <button
                              type="button"
                              className="flex min-w-0 items-center gap-1 rounded-md p-1 text-left text-[length:var(--table-label-size)] leading-normal font-medium text-muted-foreground hover:bg-surface-selected focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
            ...
            {table.getRowModel().rows.map((row) => {
              ...
              return (
                <TableRow
                  ...
                  className={cn(
                    "group/table-row border-b border-border hover:bg-surface-hover data-[state=selected]:bg-surface-selected",
