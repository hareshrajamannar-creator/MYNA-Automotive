...
const TYPE_BADGE_CLASS: Record<ContentType, string> = {
  faq:      'bg-primary/5 text-primary',
  social:   'bg-purple-50 text-purple-700',
  email:    'bg-primary/10 text-primary',
  blog:     'bg-muted text-muted-foreground',
  response: 'bg-primary/5 text-text-primary',
  ads:      'bg-surface-hover text-text-secondary',
};
            ...
            <div className="grid grid-cols-3 gap-4">
              {filtered.map((tmpl) => (
                <div
                  ...
                  className={cn(
                    'bg-background border rounded-md p-4 cursor-pointer transition-all hover:shadow-sm',
                    selected?.id === tmpl.id
                      ? 'border-primary ring-1 ring-primary/20'
                      : 'border-border hover:border-primary/40',
                  )}
                >
                  <span className={cn('inline-block text-[10px] px-2 py-0.5 rounded-md mb-2', TYPE_BADGE_CLASS[tmpl.type])}>
                    {TYPE_LABEL[tmpl.type]}
                  </span>
                  <p className="text-[13px] text-foreground mb-1">{tmpl.name}</p>
          ...
          {!selected ? (
            ...
            <>
              <div className="px-6 py-4 border-b border-border flex-shrink-0">
                <span className={cn('inline-block text-[10px] px-2 py-0.5 rounded-md mb-2', TYPE_BADGE_CLASS[selected.type])}>
